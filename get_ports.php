...
//$query=$objDB->query("SELECT state.id, state.name FROM combo INNER JOIN state ON combo.country_id = state.country_id AND combo.country_id = 1 ");

$query=$objDB->query("SELECT state.id, state.name 
FROM combo 
INNER JOIN state 
ON combo.country_id = state.country_id
WHERE combo.country_id = 1");
